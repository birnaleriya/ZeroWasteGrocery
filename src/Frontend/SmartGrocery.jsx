...
// import { db } from "../firebaseConfig";
// import { collection, addDoc, query, onSnapshot, updateDoc, doc, getDocs, orderBy, limit } from "firebase/firestore";
// import Fuse from "fuse.js";
// import jsPDF from "jspdf";
...
// const [monthlyWastageStats, setMonthlyWastageStats] = useState([]);
...
//         // Track by month
//         const wastedDate = data.timestamp?.toDate?.(); // Firestore Timestamp
//         if (wastedDate instanceof Date && !isNaN(wastedDate)) {
//           const monthKey = `${wastedDate.getFullYear()}-${String(wastedDate.getMonth() + 1).padStart(2, "0")}`;
...
//     setWastedItemsStats(sortedItems);
//     setMonthlyWastageStats(sortedMonthly); // <- Ensure this is defined via useState
...
//                 <div className="max-w-4xl mx-auto mb-8 bg-white p-4 rounded-lg shadow-md">
...
//                     <ul className="list-none pl-1 mb-4">
//                         {expiringSoon.map((item, idx) => (
//                             <li key={idx} className="flex flex-wrap items-center gap-2 mb-4 border-b pb-2">
//                                 <div className="flex-1">
//                                     <strong>{item.name}</strong> – Expiry: {item.expiryDate}
//                                     {item.expired ? (
//                                         <span className="text-red-500 font-bold ml-2">Expired</span>
//                                     ) : (
//                                         <span className="text-green-500 ml-2">{item.daysLeft} days left</span>
//                                     )}
//                                     {item.status && (
...
import React, { useState, useEffect } from "react";
import { db } from "../firebaseConfig";
import { collection, addDoc, query, onSnapshot, doc, getDocs } from "firebase/firestore";
import Fuse from "fuse.js";
import jsPDF from "jspdf";
                            ...
                            <li key={idx} className="flex flex-wrap items-center gap-2 mb-4 border-b pb-2">
                                ...
                                <div className="flex gap-2 mt-1">
                                    <input
                                        ...
                                        checked={selectedItems.includes(item.name)}
                                    />
                                    <label htmlFor={`select-${idx}`} className="text-sm">
                                        Select for recipes
                                    </label>
                                </div>
                            </li>
